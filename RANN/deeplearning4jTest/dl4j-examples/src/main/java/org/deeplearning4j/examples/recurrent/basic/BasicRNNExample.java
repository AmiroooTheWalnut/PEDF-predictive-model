...
public class BasicRNNExample {
	...
	public static void main(String[] args) {
		...
		//ADDED BY AMIROOO
        char[] MY_LEARNSTRING = "ttbuser".toCharArray();
        List<Character> MY_LEARNSTRING_CHARS_LIST = new ArrayList<>();
        // create a dedicated list of possible chars in LEARNSTRING_CHARS_LIST
        LinkedHashSet<Character> MY_LEARNSTRING_CHARS = new LinkedHashSet<>();
        for (char c : MY_LEARNSTRING)
            MY_LEARNSTRING_CHARS.add(c);
        MY_LEARNSTRING_CHARS_LIST.addAll(LEARNSTRING_CHARS);
        input = Nd4j.zeros(1, MY_LEARNSTRING_CHARS_LIST.size(), MY_LEARNSTRING.length);
        samplePos = 0;
        for (char currentChar : MY_LEARNSTRING) {
            // small hack: when currentChar is the last, take the first char as
            // nextChar - not really required. Added to this hack by adding a starter first character.
            char nextChar = MY_LEARNSTRING[(samplePos + 1) % (MY_LEARNSTRING.length)];
            // input neuron for current-char is 1 at "samplePos"
            input.putScalar(new int[] { 0, MY_LEARNSTRING_CHARS_LIST.indexOf(currentChar), samplePos }, 1);
            // output neuron for next-char is 1 at "samplePos"
            labels.putScalar(new int[] { 0, MY_LEARNSTRING_CHARS_LIST.indexOf(nextChar), samplePos }, 1);
            samplePos++;
        }
//        INDArray output = net.output(input);

        // put the first character into the rrn as an initialisation
        INDArray testInit = Nd4j.zeros(1,MY_LEARNSTRING_CHARS_LIST.size(), 1);
        testInit.putScalar(MY_LEARNSTRING_CHARS_LIST.indexOf(MY_LEARNSTRING[0]), 1);
        ...
        for (int i=0;i<40;i++) {
            ...
            // print the chosen output
            System.out.print(MY_LEARNSTRING_CHARS_LIST.get(sampledCharacterIdx));

            // use the last output as input
            INDArray nextInput = Nd4j.zeros(1, MY_LEARNSTRING_CHARS_LIST.size(), 1);
            nextInput.putScalar(sampledCharacterIdx, 1);
            output = net.rnnTimeStep(nextInput);
