...
public class MyRecurrentTrafficFineMasked {
...
//            for (int rowId = 0; rowId < input.size(1); rowId++) {
//                for (int seqId = 0; seqId < input.size(2); seqId++) {
//                    System.out.println(labelMask.getDouble(caseId, rowId, seqId));
//                }
//            }
//        }
...
//            wrapper.fit(data);
            ...
            File locationToSave = new File("MyMultiLayerNetworkMasked.zip");
...
//        newTransaction.putScalar(new int[] { 0, 55, 0 }, Math.round(input.getDouble(0,55,0)));
    ...
    public INDArray generateMask(INDArray input)
    {
        INDArray output=Nd4j.zeros(input.size(0), input.size(1), input.size(2));
        for(int i=0;i<input.size(0);i++)
        {
            for(int k=0;k<input.size(2);k++)
            {
                ...
                if(isSeqExist==true)
                {
                    for(int j=0;j<input.size(2);j++)
                    {
                        output.putScalar(new int[] { i, j, k },1);
                    }
                }
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        MyRecurrentTrafficFineMasked rnn=new MyRecurrentTrafficFineMasked();
    }
